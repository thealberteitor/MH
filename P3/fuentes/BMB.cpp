...
void BusquedaMultiarranqueBasica(int m, vector<vector<double> > dist){

  int total=10;
  vector< pair< vector<int>, double> > soluciones(total);
  pair<vector<int>, double> mejor;
  mejor.second=0;
  double coste;

  for(int i=0; i<soluciones.size(); i++){
    soluciones[i].first = SolucionInicial2(dist.size(), m);
    soluciones[i].second = CosteSolGeneral(soluciones[i].first , dist);
    EvaluaVecinos(soluciones[i], dist);

    coste = CosteSolGeneral(soluciones[i].first , dist);
    if(coste>mejor.second){
      mejor.first=soluciones[i].first;
      mejor.second=coste;
    }
  }

  cout << "COSTE SOLUCIÓN: " << CosteSolGeneral(mejor.first, dist) <<endl;
}
...
int main(int argc, char *argv[]){
  ...
  if(entrada.is_open()){
    ...
    BusquedaMultiarranqueBasica(m, distancias);
